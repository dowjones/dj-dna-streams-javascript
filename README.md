...
```bash
export USER_KEY=<Dow Jones provided user key>
# or
export OAUTH_TOKEN=<Bearer token provided by Dow Jones accounts API issuer>
# and
export SUBSCRIPTION_ID=<your stream subscription ID>
```
...
```json
{
  ...
  "oauth_token": "<Bearer token provided by Dow Jones accounts API issuer>",
...
Specifically you can pass either the service account credentials and/or subscription ID. When you start a listener you can pass the service account crendentials to the Listener constructor as an object with the field "user_key" or "oauth_token", like so:
...
const listener = new Listener({
  /**
   * Authanticate with your user key
   */
  user_key: "<YOUR USER KEY HERE>",
  /**
   * Or use an OAuth bearer token (choose just one)
   */
